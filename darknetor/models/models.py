from darknetor.utils.google_utils import *
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            modules = FeatureConcat(layers=layers)
                ...
                'type'] == 'route2':  # nn.Sequential() placeholder for 'route' layer
            layers = mdef['layers']
            filters = sum(
                [output_filters[l + 1 if l > 0 else l] for l in layers])
            routs.extend([i + l if l < 0 else l for l in layers])
            modules = FeatureConcat2(layers=layers)
                ...
                'type'] == 'route3':  # nn.Sequential() placeholder for 'route' layer
            layers = mdef['layers']
            filters = sum(
                [output_filters[l + 1 if l > 0 else l] for l in layers])
            routs.extend([i + l if l < 0 else l for l in layers])
            modules = FeatureConcat3(layers=layers)
                ...
                'type'] == 'route_lhalf':  # nn.Sequential() placeholder for 'route' layer
            layers = mdef['layers']
            filters = sum(
                [output_filters[l + 1 if l > 0 else l] for l in layers]) // 2
            routs.extend([i + l if l < 0 else l for l in layers])
            ...
            try:
                ...
                bias = bias_[:modules.no * modules.na].view(modules.na,
                                                            -1)  # shape(3,85)
                #bias[:, 4] += -4.5  # obj
                bias[:, 4] += math.log(8 / (640 / stride[yolo_index])**
                                       2)  # obj (8 objects per 640 image)
                bias[:, 5:] += math.log(
...
def convert(cfg='cfg/yolov3-spp.cfg',
