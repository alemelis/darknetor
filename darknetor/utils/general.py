...
from darknetor.utils.torch_utils import init_seeds, is_parallel
...
def build_targets(p, targets, model):
    ...
    for i, jj in enumerate(
        ...
        anchors = model.module.module_list[
            jj].anchor_vec if multi_gpu else model.module_list[jj].anchor_vec
        ...
        if nt:
            ...
            j = torch.max(r,
                          1. / r).max(2)[0] < model.hyp['anchor_t']  # compare
...
def strip_optimizer(f='weights/best.pt',
                    s=''):  # from utils.utils import *; strip_optimizer()
...
def increment_dir(dir, comment=''):
    ...
    if len(d):
        n = max(
            [int(x[len(dir):x.find('_') if '_' in x else None])
