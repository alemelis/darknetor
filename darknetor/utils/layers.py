...
from darknetor.utils.general import *
...
class FeatureConcat3(nn.Module):
    ...
    def forward(self, x, outputs):
        return torch.cat([
            outputs[self.layers[0]], outputs[self.layers[1]].detach(),
...
class FeatureConcat_l(nn.Module):
    ...
    def forward(self, x, outputs):
        ...
        ], 1) if self.multiple else outputs[
            self.layers[0]][:, :outputs[self.layers[0]].shape[1] // 2, :, :]
